# Knowledge Catalog for Retailers: Acme Retail Demo

Welcome to the Acme Retail Demo. This project demonstrates how to build a deep universal context engine for AI agents using Google Cloud's Knowledge Catalog. By shifting from passive metadata management to an active context layer, this demo showcases how to establish an always-on, authoritative grounding layer for enterprise data.

## Demo Intent & Use Cases

This repository captures several core capabilities of using Knowledge Catalog for a retailer:

### 1. Eliminating Data Silos & Dark Data
Achieve total visibility across the enterprise without data movement. This demo breaks the "dark data barrier" by automatically crawling unstructured PDF recipes and product manuals, integrating multi-cloud Iceberg tables (via the Governed Lakehouse), and connecting zero-copy SaaS applications (like Salesforce) to create a single source of truth.

### 2. Building the 'Trust Layer' for AI
Turn raw, noisy data into usable context. Instead of manual tagging, this demo utilizes Gemini to autonomously generate business glossaries and data insights, creating a self-governing data estate powered by AI-driven overviews, automated data quality scans, and custom aspect types.

### 3. Grounding AI for Precise Action
Eliminate AI hallucinations by grounding agents in "Verified Truth." Agents can use the Knowledge Catalog to perform impact analysis before changes are made or discover certified datasets through high-precision semantic search and context APIs.

### 4. Accelerating Innovation with Curated Data Products
Enable teams to rapidly discover, understand, and consume governed, high-quality data assets. Data-producing teams can package related data entities, along with their associated metadata (ownership, access policies, SLAs, glossary terms), into discoverable Data Products.

### 5. Enterprise Governance, Trust, and Observability
Establish a foundation of trust with real-time ACL-aware retrieval (ensuring agents only see what they have permission to see), human-in-the-loop validation workflows, and comprehensive lineage and explainability auditing for every AI action.
